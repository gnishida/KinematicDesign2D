		...
		for (int iter = 0; iter < num_iterations; iter++) {
			...
			for (int i = 0; i < new_particles.size(); i++) {
				...
				if (optimizeCandidate(new_particles[i].solution.poses, linkage_region_pts, linkage_region_bbox, new_particles[i].solution.points)) {
					...
					if (checkHardConstraints(new_particles[i].solution.points, new_particles[i].solution.poses, linkage_region_pts, linkage_avoidance_pts, fixed_bodies, moving_body)) {
						// calculate the score
