...
namespace kinematics {
	...
	public:
		LinkageSynthesisRRRP(const std::vector<Object25D>& fixed_bodies, const std::pair<double, double>& sigmas, bool avoid_branch_defect, double min_transmission_angle, double min_link_length, const std::vector<double>& weights);
	...
	public:
		...
		bool optimizeCandidate(const std::vector<glm::dmat3x3>& poses, std::vector<glm::dvec2>& points);
		bool optimizeLink(const std::vector<glm::dmat3x3>& poses, glm::dvec2& A0, glm::dvec2& A1);
		bool optimizeSlider(const std::vector<glm::dmat3x3>& poses, glm::dvec2& A0, glm::dvec2& A1);
		Solution findBestSolution(const std::vector<glm::dmat3x3>& poses, std::vector<Solution>& solutions, const cv::Mat& dist_map, const BBox& dist_map_bbox, const std::vector<glm::dvec2>& linkage_avoidance_pts, const Object25D& moving_body, int num_particles, int num_iterations, bool record_file);
		double calculateCost(Solution& solution, const Object25D& moving_body, const cv::Mat& dist_map, const BBox& dist_map_bbox);
		Kinematics constructKinematics(const std::vector<glm::dmat3x3>& poses, const std::vector<glm::dvec2>& points, const Object25D& moving_body, bool connect_joints, const std::vector<Object25D>& fixed_bodies, std::vector<glm::dvec2>& connected_pts);
		void updateMovingBodies(Kinematics& kinematics, const Object25D& moving_body);

		bool checkHardConstraints(std::vector<glm::dvec2>& points, const std::vector<glm::dmat3x3>& poses, const std::vector<glm::dvec2>& linkage_avoidance_pts, const Object25D& moving_body);
